...
export default function AIAnalysis({ entry, onRefresh, onUpdateEntry }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Analysis Summary - Removed status sections */}
      ...
      {/* Reflection Prompts */}
      {(hasPrompts || (hasPatternRecognition && entry.ai_analysis.pattern_recognition.investigation_questions)) && (
        <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
          <h4 className="font-medium text-gray-900 dark:text-white mb-3 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {/* Regular prompts */}
            {hasPrompts && entry.ai_prompts.map((prompt, index) => {
              ...
              return (
                <div key={promptId} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                  {/* Prompt Header */}
                  <div className="p-3">
            ...
            {/* Investigation Questions from Pattern Recognition */}
            {hasPatternRecognition && entry.ai_analysis.pattern_recognition.investigation_questions && entry.ai_analysis.pattern_recognition.investigation_questions.length > 0 && (
              <>
                {entry.ai_analysis.pattern_recognition.investigation_questions.map((question, index) => {
                  const promptId = `investigation_${index}`
                  const isExpanded = expandedPrompts[promptId]
                      ...
                      <div className="p-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="px-2 py-1 bg-orange-100 dark:bg-orange-800 text-orange-700 dark:text-orange-300 text-xs rounded-full">
                        ...
                        <div className="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700">
                          ...
                          <div className="p-3 space-y-3">
                            ...
                            <textarea
                              value={responses[promptId] || ''}
                              onChange={(e) => handleResponseChange(promptId, e.target.value)}
                              placeholder="Share your thoughts, insights, or experiences related to this pattern investigation question..."
                              rows={3}
                              className="w-full p-3 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 resize-none"
                            />
                            <div className="flex justify-end">
