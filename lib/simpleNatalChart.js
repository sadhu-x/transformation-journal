...
function getAyanamsa(jd) {
  ...
  // Lahiri ayanamsa formula - adjusted for better accuracy
  return 23.85 + 0.000117 * t * t - 0.000000002 * t * t * t + 0.0000000001 * t * t * t * t
}
...
// Calculate other planets (more accurate ephemeris)
function getPlanetPosition(jd, planet) {
  ...
  switch (planet) {
    case 'mars':
      // Mars ephemeris
      const L_mars = 355.433275 + 19141.6964746 * t - 0.0003106 * t * t + t * t * t / 153400
      const M_mars = 19.3730 + 19141.6964746 * t - 0.0003106 * t * t + t * t * t / 153400
      const e_mars = 0.0934052 + 0.0000002 * t
      const C_mars = (10.6901 + 0.0008 * t) * Math.sin(M_mars * Math.PI / 180) +
                     (0.0108 + 0.0001 * t) * Math.sin(2 * M_mars * Math.PI / 180) +
                     0.0002 * Math.sin(3 * M_mars * Math.PI / 180)
      return L_mars + C_mars
      
    case 'mercury':
      // Mercury ephemeris
      const L_mercury = 252.250906 + 149472.6746358 * t - 0.0005363 * t * t + t * t * t / 24490000
      const M_mercury = 168.6562 + 149472.6746358 * t - 0.0005363 * t * t + t * t * t / 24490000
      const e_mercury = 0.2056318 + 0.0000002 * t
      const C_mercury = (23.4400 + 0.0001 * t) * Math.sin(M_mercury * Math.PI / 180) +
                        (2.9818 + 0.0004 * t) * Math.sin(2 * M_mercury * Math.PI / 180) +
                        (0.5255 + 0.0002 * t) * Math.sin(3 * M_mercury * Math.PI / 180) +
                        0.1058 * Math.sin(4 * M_mercury * Math.PI / 180) +
                        0.0241 * Math.sin(5 * M_mercury * Math.PI / 180)
      return L_mercury + C_mercury
      
    case 'jupiter':
      // Jupiter ephemeris
      const L_jupiter = 34.351519 + 3034.9056606 * t - 0.0000850 * t * t + t * t * t / 24490000
      const M_jupiter = 20.0202 + 3034.9056606 * t - 0.0000850 * t * t + t * t * t / 24490000
      const e_jupiter = 0.0484984 + 0.0000002 * t
      const C_jupiter = (5.2083 + 0.0001 * t) * Math.sin(M_jupiter * Math.PI / 180) +
                        (0.2516 + 0.0001 * t) * Math.sin(2 * M_jupiter * Math.PI / 180) +
                        0.0063 * Math.sin(3 * M_jupiter * Math.PI / 180) +
                        0.0027 * Math.sin(4 * M_jupiter * Math.PI / 180)
      return L_jupiter + C_jupiter
      
    case 'venus':
      // Venus ephemeris
      const L_venus = 181.979801 + 58517.8153876 * t + 0.0003106 * t * t + t * t * t / 24490000
      const M_venus = 212.6032 + 58517.8153876 * t + 0.0003106 * t * t + t * t * t / 24490000
      const e_venus = 0.0067733 + 0.0000002 * t
      const C_venus = (0.7233 + 0.0001 * t) * Math.sin(M_venus * Math.PI / 180) +
                      (0.0003 + 0.0001 * t) * Math.sin(2 * M_venus * Math.PI / 180) +
                      0.0001 * Math.sin(3 * M_venus * Math.PI / 180)
      return L_venus + C_venus
      
    case 'saturn':
      // Saturn ephemeris
      const L_saturn = 50.077471 + 1222.1137943 * t + 0.0002104 * t * t + t * t * t / 24490000
      const M_saturn = 317.0207 + 1222.1137943 * t + 0.0002104 * t * t + t * t * t / 24490000
      const e_saturn = 0.0545090 + 0.0000002 * t
      const C_saturn = (6.2555 + 0.0001 * t) * Math.sin(M_saturn * Math.PI / 180) +
                       (0.4139 + 0.0001 * t) * Math.sin(2 * M_saturn * Math.PI / 180) +
                       0.0275 * Math.sin(3 * M_saturn * Math.PI / 180) +
                       0.0188 * Math.sin(4 * M_saturn * Math.PI / 180) +
                       0.0063 * Math.sin(5 * M_saturn * Math.PI / 180)
      return L_saturn + C_saturn
...
function getLunarNodes(jd) {
  ...
  // Normalize to 0-360 degrees
  let normalizedRahu = rahu % 360
  if (normalizedRahu < 0) normalizedRahu += 360
  
  // Ketu is always 180° opposite to Rahu
  const ketu = (normalizedRahu + 180) % 360
  
  return { rahu: normalizedRahu, ketu }
}
