...
const API_ASTRO_URL = 'https://json.apiastro.com/planets'
...
// Convert API Astro data to our format
function formatApiAstroData(apiData) {
  try {
    console.log('Formatting API Astro data:', apiData)
    
    if (!apiData || !apiData.output || !Array.isArray(apiData.output) || apiData.output.length === 0) {
      console.error('Invalid API Astro data structure')
      return null
    }
    
    // Use the second output format which is cleaner
    const planetaryData = apiData.output[1] || apiData.output[0]
    ...
    Object.keys(planetaryData).forEach(key => {
      ...
      if (planetData && typeof planetData === 'object' && planetData.current_sign !== undefined) {
        const planet = {
          planet: key,
          rashi: ZODIAC_SIGNS[planetData.current_sign - 1] || 'Unknown',
        ...
        // Calculate nakshatra for Moon (lunar mansion)
        if (key === 'Moon') {
          planet.nakshatra = calculateNakshatra(planetData.fullDegree)
        } else {
          planet.nakshatra = 'Unknown' // We'll calculate this later if needed
        }
        ...
        if (key === 'Ascendant') {
          ascendant = {
            ...
            nakshatra: calculateNakshatra(planetData.fullDegree),
            longitude: planetData.fullDegree,
            longitude_dms: `${Math.floor(planetData.fullDegree)}° ${Math.floor((planetData.fullDegree % 1) * 60)}' ${Math.floor(((planetData.fullDegree % 1) * 60 % 1) * 60)}"`,
            sign_degree_dms: `${Math.floor(planetData.normDegree)}° ${Math.floor((planetData.normDegree % 1) * 60)}' ${Math.floor(((planetData.normDegree % 1) * 60 % 1) * 60)}"`
          }
        } else {
          planets.push(planet)
        }
