...
async function tryProkeralaAPI(day, month, year, hour, minute, latitude, longitude) {
  try {
    ...
    console.log('Calling server-side astrology API...')
    ...
    if (response.ok) {
      const data = await response.json()
      console.log('Server-side astrology API response:', data)
      return data
    } else {
      console.warn('Server-side astrology API failed:', response.status, response.statusText)
    }
