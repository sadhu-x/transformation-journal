...
// Free API options:
// 1. Prokerala API (free with limits) - USER'S CHOICE
// 2. Vedic Rishi API (free tier available)
...
// - Prokerala: https://prokerala.com/astrology/api/ (USER'S CHOICE)
...
export async function getNatalChartAPI(birthDate, birthTime, latitude, longitude) {
  try {
    ...
    // Try APIs in order of preference (Prokerala first since user chose it)
    const apis = [
...
// Prokerala API (free with limits) - USER'S CHOICE
async function tryProkeralaAPI(day, month, year, hour, minute, latitude, longitude) {
  try {
    // Prokerala uses a different format - we need to use their planetary positions endpoint
    ...
    if (response.ok) {
      ...
      console.warn('Prokerala API error response:', response.status, response.statusText)
    }
  } catch (error) {
