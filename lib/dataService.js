...
export const addEntry = async (entry) => {
  ...
  try {
    ...
    // Trigger Claude analysis if content is substantial
    if (entry.content && entry.content.trim().length > 50) {
      try {
        console.log('🤖 Triggering Claude analysis for new entry...')
        const analysisResult = await analyzeEntryWithClaude(entry.content, insertedEntry.id)
        
        if (analysisResult.success) {
          // Update the entry with AI analysis results
          await updateEntry(insertedEntry.id, {
        ...
        console.warn('⚠️ Claude analysis failed, but entry was saved:', analysisError.message)
      }
    }
