...
import { getNatalChartAPI } from './astrologyAPI.js'
...
export const generateInstructionTemplate = async (userConfig = {}) => {
  ...
  console.log('Checking birth data for natal chart calculation:', {
    birthDate,
    birthTime,
    birthLatitude,
    birthLongitude
  ...
  if (birthDate && birthDate !== '[Not provided]' && birthDate.trim() !== '' &&
...
export const saveBirthChartData = async (birthDate, birthTime, birthLatitude, birthLongitude, chartData) => {
  try {
    ...
    console.log('Successfully saved birth chart data to Supabase')
    return data
  } catch (error) {
...
export const fetchAndStoreBirthChart = async (birthDate, birthTime, birthLatitude, birthLongitude) => {
  try {
    // First check if we already have this birth chart data
    const existingData = await getBirthChartData(birthDate, birthTime, birthLatitude, birthLongitude)
    if (existingData) {
      console.log('Birth chart data already exists in database')
      return {
        success: true,
        data: existingData,
        cached: true
    ...
    // Fetch new birth chart data from API
    console.log('Fetching birth chart data from API...')
    const chartData = await getNatalChartAPI(birthDate, birthTime, birthLatitude, birthLongitude)
