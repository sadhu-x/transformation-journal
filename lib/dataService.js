...
export const generateInstructionTemplate = (userConfig = {}) => {
  const {
...
## Context
I'm tracking my daily consciousness, emotions, and patterns for personal transformation using a custom journal app. I'd like you to analyze this data to identify patterns, insights, and recommendations for optimizing my growth.
...
3. **Actionable Recommendations**: Specific changes to optimize my practices
4. **Pattern Recognition**: What patterns does this entry reveal?
5. **Questions for Deeper Investigation**: What should I track more closely?`
}
...
export const exportData = async () => {
  try {
    ...
    // Convert image URLs back to base64 for export
    const exportData = await Promise.all(
      entries.map(async (entry) => {
        if (entry.attachments) {
          ...
          return { ...entry, attachments: processedAttachments }
        }
        return entry
      })
    )
...
export const importData = async (entries) => {
  try {
    // Clear existing data
    ...
    // Import new entries
    const importPromises = entries.map(entry => addEntry(entry))
...
export const getNonNegotiables = async () => {
  if (!isSupabaseAvailable()) {
    console.warn('Supabase not available, using localStorage')
