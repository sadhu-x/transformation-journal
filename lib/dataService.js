...
// Save entry without AI analysis
export const saveEntry = async (entry) => {
  ...
  try {
    ...
    console.log('✅ Entry saved to Supabase without AI analysis')
...
// Trigger AI analysis for an existing entry
export const triggerAIAnalysis = async (entryId, content) => {
  if (!content || content.trim().length < 50) {
    console.warn('Content too short for AI analysis')
    return { success: false, error: 'Content too short for analysis' }
  }

  try {
    console.log('🤖 Triggering Claude analysis for entry:', entryId)
    const analysisResult = await analyzeEntryWithClaude(content, entryId)
    ...
    if (analysisResult.success) {
      ...
      console.log('✅ Claude analysis completed and saved')
...
// Legacy function - now calls saveEntry (for backward compatibility)
...
// Claude AI Analysis
export const analyzeEntryWithClaude = async (content, entryId = null) => {
  try {
...
export const testClaudeConnection = async () => {
  try {
    const response = await fetch('/api/claude?action=test')
    if (!response.ok) {
      throw new Error('Failed to test Claude connection')
    }
    return await response.json()
  } catch (error) {
